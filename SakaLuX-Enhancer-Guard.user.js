...
`;
  ...
  const SELF=Object.assign({"id":"enhancer","name":"Enhancer","icon":"🛡️","selector":"","fallback":"https://www.torn.com/item.php"},{version:'1.3.51'});
...
(function () {
    ...
    function isProtectedSalePage() {
        ...
        return href.includes('#/addlisting')
            ...
            || href.includes('bazaar.php')
            || href.includes('trade.php');
    }

    function saleRows() {
        const rows = new Set(document.querySelectorAll('li.clearfix, .sell-items-list > li, [class*="virtualListing___"]'));
        document.querySelectorAll('input[placeholder="Qty" i], input[placeholder="Price" i], input.input-money').forEach(control => {
            let node = control;
            for (let depth = 0; depth < 7 && node?.parentElement; depth++) {
                node = node.parentElement;
                const hasImage = Boolean(node.querySelector('img'));
                const hasSaleControl = Boolean(node.querySelector('input[placeholder="Qty" i], input[placeholder="Price" i], input.input-money'));
                if (hasImage && hasSaleControl) { rows.add(node); break; }
            }
        });
        return [...rows];
    }

    function fullyProtectedSaleRow(row, locks = readProtectorLocks()) {
        const name = saleRowName(row);
    ...
    function installSaleProtectionFallback() {
        ...
        document.addEventListener('click', event => {
            if (!isProtectedSalePage()) return;
            const control = event.target?.closest?.('button, [role="button"], input[type="checkbox"]');
            if (!control) return;
            const row = saleRows().find(candidate => candidate.contains(control));
...
/* Compact donation controls and Elimination mobile panel geometry 1.3.45 */
(()=>{const s=document.createElement('style');s.textContent="@media(max-width:820px){\n#sl-eg-overlay#sl-eg-overlay#sl-eg-overlay{position:fixed!important;inset:0 4px 36px!important;top:0!important;bottom:36px!important;left:4px!important;right:4px!important;width:auto!important;height:auto!important;min-width:0!important;min-height:0!important;max-width:none!important;max-height:none!important;margin:0!important;transform:none!important;box-sizing:border-box!important;padding:0!important;background:transparent!important;overflow:hidden!important;border-radius:14px!important;align-items:stretch!important;justify-content:stretch!important;}\n#sl-eg-overlay#sl-eg-overlay#sl-eg-overlay #sl-eg-panel#sl-eg-panel{position:relative!important;inset:auto!important;top:auto!important;bottom:auto!important;left:auto!important;right:auto!important;align-self:stretch!important;flex:1 1 auto!important;width:100%!important;height:100%!important;min-height:0!important;max-height:100%!important;max-width:100%!important;margin:0!important;transform:none!important;box-sizing:border-box!important;border:1px solid #3c4652!important;border-radius:14px!important;}\n#sl-eg-overlay#sl-eg-overlay#sl-eg-overlay #sl-eg-panel#sl-eg-panel{display:flex!important;flex-direction:column!important;overflow:hidden!important;}\n#sl-eg-overlay#sl-eg-overlay#sl-eg-overlay #sl-eg-panel#sl-eg-panel>#sl-eg-list{flex:1 1 auto!important;min-height:0!important;overflow-y:auto!important;overscroll-behavior:contain!important;}\n\n}";(document.head||document.documentElement).appendChild(s)})();
