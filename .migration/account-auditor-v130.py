...
# Increase default history coverage.
text=text.replace("includePrivateData:true, maxPrivatePages:5,", "includePrivateData:true, maxPrivatePages:200,", 1)

# Replace legacy duplicate endpoint definitions with canonical v2-first set.
...
# Replace pagination with high-coverage, loop-safe collector.
...
# Private collectors should pass explicit history limit.
...
# Replace snapshot collector completely: v2 canonical, v1 removed, full pagination.
...
# Avoid GitHub commits when exact file content is unchanged.
...
# Replace split/sync block so each data field is stored once.
...
# Update standalone documentation only; Auditor stays absent from Hub registry.
