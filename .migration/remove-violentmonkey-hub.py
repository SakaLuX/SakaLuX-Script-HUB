...
# Add clean release entry without naming any specific userscript manager.
...
# Remove historical manager-specific v1.9.10 changelog object entirely.
s, n = re.subn(
    r"\n\s*\{\n\s*version: '1\.9\.10',\n\s*date: '2026-09-11',\n\s*changes: \[.*?\n\s*\]\n\s*\},",
    '', s, count=1, flags=re.S
)
if n != 1:
    raise SystemExit('v1.9.10 compatibility changelog block not found')
...
# Remove the special installed-but-no-bridge manager-specific alert branch.
...
"""
if old not in s:
    raise SystemExit('manager-specific OPEN alert branch not found')
s = s.replace(old, new, 1)

if re.search(r'violentmonkey', s, flags=re.I):
    raise SystemExit('manager-specific name still present in Hub userscript')
...
if re.search(r'violentmonkey', d, flags=re.I):
    raise SystemExit('manager-specific name still present in Hub documentation')
doc.write_text(d, encoding='utf-8')

print('Removed manager-specific Hub handling; Hub v1.9.34')
