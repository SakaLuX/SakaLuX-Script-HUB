...
if '// @version      1.9.42' not in s or "const VERSION = '1.9.42';" not in s:
    raise SystemExit('Unexpected Hub baseline; refusing unsafe integration')
s = s.replace('// @version      1.9.42', '// @version      1.9.43', 1)
s = s.replace("const VERSION = '1.9.42';", "const VERSION = '1.9.43';", 1)
...
'''
open_marker = '    function openHub() {\n'
if open_marker not in s:
    raise SystemExit('openHub marker missing')
s = s.replace(open_marker, helpers + open_marker, 1)
s = s.replace("    function openHub() {\n        const registryClass",
              "    function openHub() {\n        ensureNativeCardStyles();\n        startManagedFooterObserver();\n        const registryClass", 1)

new_render = r'''    function renderCard(row) {
...
pattern = re.compile(r'    function renderCard\(row\) \{.*?\n    function bindCards\(\) \{', re.S)
s, count = pattern.subn(lambda _: new_render, s, count=1)
if count != 1:
    raise SystemExit(f'renderCard patch count={count}')

s = s.replace("                const result = await api[action.method]();\n",
...
d = d.replace('## Current version\n**v1.9.42**', '## Current version\n**v1.9.43**', 1)
d = re.sub(r'(## Current release note\s*\n+)\*\*v1\.9\.42\*\*[^\n]*',
           r'\1**v1.9.43** Native module-card release center: adds per-module INFO/NEW controls, compact status-only cards, Greasy Fork public version checks, and stable SakaLuX panel footers.', d, count=1)
release_entry = '''### v1.9.43 — Native module information and release center
...
d = d.replace(history, history + release_entry, 1)
d = d.replace('- Gives installed modules a native ON/OFF switch plus one OPEN or SETTINGS action.\n',
              '- Gives installed modules native INFO, NEW, ON/OFF and OPEN/SETTINGS controls.\n', 1)
doc.write_text(d, encoding='utf-8')

json.loads(Path('scripts.json').read_text(encoding='utf-8'))
Path('SakaLuX-Hub-Card-UX.user.js').unlink(missing_ok=True)
