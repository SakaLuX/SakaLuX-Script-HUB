...
text,n=re.subn(old_css,lambda m:new_css,text,count=1)
...
rep_ctx=r"""    function getResourceBarContext() {
        const visible = el => {
...
old_setting="${settingSwitch('slhs-topbar', 'Torn-native HUB launcher', 'Show the blinking skull HUB entry before Messages when Torn navigation is available.', settings.showTopbarSkull)}"
new_setting="${settingSwitch('slhs-topbar', 'Torn resource-bar launcher', 'Show the compact SakaLuX button before the money resource when available. The skull button is used automatically as fallback.', settings.showTopbarSkull)}"
if old_setting not in text: raise SystemExit('settings launcher copy not found')
text=text.replace(old_setting,new_setting,1)

old_check="results.push({ level: document.getElementById(IDS.topSkull) ? 'ok' : 'warn', label: 'Torn-native HUB launcher', detail: document.getElementById(IDS.topSkull) ? 'Mounted as a native mobile navigation entry before Messages' : 'Torn mobile navigation not detected yet' });"
new_check="results.push({ level: document.getElementById(IDS.topSkull) ? 'ok' : 'warn', label: 'Torn resource-bar HUB launcher', detail: document.getElementById(IDS.topSkull) ? 'Mounted before the money resource' : 'Resource bar not detected — floating skull fallback active' });"
if old_check not in text: raise SystemExit('system check wording not found')
