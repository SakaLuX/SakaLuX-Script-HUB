...
const KEYS={
  ...
  myStats:'slx_elim_my_stats_v1',enabled:'slx_elim_enabled'
};
...
function shouldOfferHub(){if(!state.enabled||isHubInstalled())return false;try{const last=Number(localStorage.getItem(HUB_PROMPT_STORAGE)||0);return !last||Date.now()-last>=HUB_PROMPT_INTERVAL}catch{return true}}
...
function inject(){
  if(!state.enabled||document.getElementById(IDS.panel))return;injectCSS();
