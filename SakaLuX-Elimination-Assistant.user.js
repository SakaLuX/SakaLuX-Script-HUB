...
function inject(){if(!state.enabled||$('#'+IDS.panel))return;css();compactCss();const b=document.createElement('button');b.id=IDS.button;b.textContent='ELIM ⚔';const p=document.createElement('div');p.id=IDS.panel;p.innerHTML=`<div class="slx-h"><b>SakaLuX Elimination Assistant v${VERSION}</b><button id="slx-set">⚙</button><button id="slx-close">✕</button></div><div class="slx-t slx-top"><select id="slx-team"><option value="">Team…</option></select><button id="slx-load">LOAD NEXT</button><button id="slx-ff">FF SCAN</button><button id="slx-export" title="Export Targets for TornPDA">EXPORT TARGETS</button></div><div class="slx-t slx-search"><div class="slx-filter-chips" id="slx-filters"><label class="slx-filter-chip"><input type="checkbox" data-filter="attackable"><span>ATTACKABLE</span></label><label class="slx-filter-chip safe-chip"><input type="checkbox" data-filter="safe"><span>SAFE</span></label><label class="slx-filter-chip risky-chip"><input type="checkbox" data-filter="risky"><span>RISKY</span></label><label class="slx-filter-chip skip-chip"><input type="checkbox" data-filter="skip"><span>SKIP</span></label><label class="slx-filter-chip"><input type="checkbox" data-filter="unopened"><span>UNOPENED</span></label></div><input id="slx-q" placeholder="Player / ID"></div><div class="slx-s"><span id="slx-status">${getTornKey()?'API key ready.':'Set a Torn API key in Settings or Hub.'}</span><button class="slx-cal-mini" id="slx-cal">CALIBRATE</button><b id="slx-summary"></b></div><div class="slx-body"><table><colgroup><col class="signal"><col class="player"><col class="lvl"><col class="last"><col class="actions"></colgroup><thead><tr><th>Signal</th><th>Player / Status</th><th>Lvl</th><th>Last</th><th>Actions</th></tr></thead><tbody id="slx-rows"></tbody></table></div><div class="slx-modal" id="slx-settings"><h3>Settings</h3><p>${window.SakaLuXScriptHub?.getApiKey?.()?'Using the shared SakaLuX Hub Torn API key. ':''}TEST TORN KEY checks battlestats and Eliminations separately. API 32 on eliminationteam means that Torn is not currently exposing that event endpoint; it is not treated as a bad API key.</p><button class="slx-a" id="slx-create">🔑 ${window.SakaLuXScriptHub?'CREATE GENERAL HUB KEY':'CREATE REQUIRED TORN KEY'}</button> <button class="slx-a" id="slx-test">🧪 TEST TORN KEY</button><label>Standalone Torn API key</label><input id="slx-torn"><label>FFScouter API key (optional, separate service)</label><input id="slx-ffkey"><label>Manual total BS</label><input id="slx-manual" type="number"><button class="slx-a" id="slx-save">SAVE</button> <button class="slx-a" id="slx-cancel">CLOSE</button></div>`;document.body.append(b,p);guardAgainstForeignTargetWidgets();b.onclick=()=>{p.classList.toggle('open');guardAgainstForeignTargetWidgets()};$('#slx-close',p).onclick=()=>p.classList.remove('open');$('#slx-set',p).onclick=openSettings;$('#slx-cancel',p).onclick=()=>$('#slx-settings',p).classList.remove('open');$('#slx-save',p).onclick=saveSettings;$('#slx-create',p).onclick=createKey;$('#slx-test',p).onclick=()=>busy(testKey);$('#slx-load',p).onclick=()=>busy(loadNextTeamBatch);$('#slx-ff',p).onclick=()=>busy(()=>loadFF(true));$('#slx-export',p).onclick=()=>busy(exportTargets);$('#slx-cal',p).onclick=()=>busy(()=>calibrate(true));$('#slx-q',p).oninput=apply;$$('#slx-filters input[data-filter]',p).forEach(x=>x.onchange=apply);$('#slx-team',p).onchange=e=>{state.teamId=Number(e.target.value||0);localStorage.setItem(K.team,String(state.teamId||''));state.players=[];state.view=[];state.batchRawCount=0;render();setStatus('Team selected · press LOAD NEXT')};if(getTornKey())busy(refresh)}
