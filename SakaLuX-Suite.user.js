...
const MODULES=[
{id:'daily-prayer',name:'Daily Prayer Bell',icon:'🙏',cat:'Reminders',desc:'Daily prayer cue with UTC reset, Torn Church shortcut and automatic success detection.',kind:'internal',enabled:false,action:'prayer',settings:true},
...
{id:'item-signals',name:'Item Signals',icon:'🔎',cat:'Reminders',desc:'Smart purpose markers for OC gear, crime tools, boosters, medical items and enhancers.',kind:'internal',enabled:false,action:'items',settings:true},
{id:'event-lens',name:'Event Lens',icon:'🗂️',cat:'Reminders',desc:'Searchable event intelligence with categories, counts and locally saved events.',kind:'internal',enabled:false,action:'events',settings:true},
{id:'chain-alarm',name:'Chain Alarm',icon:'⛓️',cat:'Reminders',desc:'Floating chain timer with warning thresholds, drag support and critical alerts.',kind:'internal',enabled:false,action:'chain',settings:true},
...
function warPerformance(){
 ...
 const resultText=(document.body.innerText||'').match(/ranked war[\s\S]{0,250}?(victory|won|defeat|lost)/i);d.history??=[];if(resultText){const outcome=/victory|won/i.test(resultText[1])?'W':'L',key=location.pathname+location.search+location.hash+'|'+outcome;if(!d.history.some(x=>x.key===key))d.history.unshift({key,outcome,at:Date.now()});d.history=d.history.slice(0,200)}
