...
// @author       SakaLuX
// @match        https://www.torn.com/page.php?sid=missions*
...
(function () {
        ...
        return new Promise((resolve, reject) => {
            if (typeof window.PDA_httpGet === 'function') {
                ...
                window.PDA_httpGet(url, { Accept: 'application/json' })
                    .then(response => {
                        try { resolve(parseApiResponse(response)); } catch (error) { reject(error); }
                    })
                    .catch(reject);
                return;
            }
    ...
    async function loadAmmo(force = false) {
        ...
        try {
            const url = 'https://api.torn.com/user/?selections=ammo&key=' + encodeURIComponent(key);
            const data = await apiGet(url);
            checkApiError(data);
    ...
    function getModRange(name) {
        const ranges = getModRanges();
        return ranges[normalize(name)] || null;
    }
    ...
    function findCatalogueItem(data) {
        ...
        if (data?.name) return state.catalogue.get(normalize(data.name)) || null;
        return null;
    }
    ...
    function renderItemBadge(box, data) {
        ...
        box.innerHTML = `
            <div class="sl-mr-line">💰 ${formatMoney(totalValue)}</div>
            <div class="sl-mr-line good">${formatMoney(perCredit)} / credit</div>
        `;
    }
    ...
    function renderAmmoBadge(box, data) {
        ...
        box.innerHTML = `
            <div class="sl-mr-line">🔫 Owned: ${formatNumber(owned)}</div>
            <div class="sl-mr-line muted">${formatNumber(data.amount || 0)} for ${formatNumber(data.points || 0)} credits</div>
        `;
    }
    ...
    function renderModBadge(box, data) {
        ...
        let normalText = 'No normal range yet';
        let specialText = 'No special range yet';
        if (range?.min != null) normalText = range.min === range.max ? String(range.min) : range.min + '–' + range.max;
        if (range?.specialMin != null) specialText = range.specialMin === range.specialMax ? String(range.specialMin) : range.specialMin + '–' + range.specialMax;
        box.innerHTML = `
            <div class="sl-mr-line ${special ? 'special' : ''}">${special ? '⭐ SPECIAL' : '🧩 MOD'} • ${formatNumber(data.points || 0)} credits</div>
            <div class="sl-mr-line muted">Seen: ${escapeHtml(normalText)}</div>
            <div class="sl-mr-line special-text">Special: ${escapeHtml(specialText)}</div>
        `;
    }
    ...
    function getActiveReward() {
        ...
        if (active) return { element: active, data: parseRewardData(active) };
        return null;
    }

    function removeDetailPanel() {
        document.querySelectorAll('.sl-mr-detail').forEach(element => element.remove());
    }
    ...
    async function renderDetailPanel() {
        ...
        panel.innerHTML = `
            <div class="sl-mr-detail-title">🎯 SakaLuX Reward Info</div>
            <div class="sl-mr-detail-body">Loading...</div>
        `;
        ...
        if (type === 'item') {
            ...
            if (item?.marketPrice) {
                ...
                body.innerHTML = `
                    <div>Market value: <b>${formatMoney(item.marketPrice)}</b></div>
                    <div>Reward amount: <b>${formatNumber(amount)}</b></div>
                    <div>Estimated total: <b>${formatMoney(total)}</b></div>
                    <div class="sl-mr-highlight">💰 Value / credit: <b>${formatMoney(perCredit)}</b></div>
                `;
            } else {
                body.textContent = 'Market value unavailable.';
            }
            return;
        }
        ...
        if (type === 'ammo') {
            ...
            const owned = getOwnedAmmo(data.ammoType, data.name);
            body.innerHTML = `
                <div>Ammo: <b>${escapeHtml(data.name)}</b></div>
                <div>Type: <b>${escapeHtml(data.ammoType || '?')}</b></div>
                <div>Reward: <b>${formatNumber(data.amount || 0)}</b></div>
                <div>Cost: <b>${formatNumber(data.points || 0)} credits</b></div>
                <div class="sl-mr-highlight">🔫 Currently owned: <b>${formatNumber(owned)}</b></div>
            `;
            return;
        }
        ...
        if (type === 'mod') {
            ...
            const range = getModRange(data.name);
            body.innerHTML = `
                <div>Mod: <b>${escapeHtml(data.name)}</b></div>
                <div>Current cost: <b>${formatNumber(data.points || 0)} credits</b></div>
                <div>Offer: <b>${special ? '⭐ SPECIAL' : 'Normal'}</b></div>
                <br>
                <div>Observed normal range: <b>${range?.min != null ? range.min + (range.max !== range.min ? ' – ' + range.max : '') : 'Not enough data'}</b></div>
                <div>Observed special range: <b>${range?.specialMin != null ? range.specialMin + (range.specialMax !== range.specialMin ? ' – ' + range.specialMax : '') : 'Not enough data'}</b></div>
                <div class="sl-mr-note">Ranges are learned locally from Mission Shop offers seen on this device.</div>
            `;
            return;
        }

        body.innerHTML = 'No additional information available for this reward type.';
    }
    ...
    async function scanRewards(force = false) {
        ...
        for (const card of cards) {
            ...
            try {
                ...
                console.error('[SakaLuX Mission Rewards]', error);
            }
        }
    ...
    function openSettings() {
        ...
        overlay.innerHTML = `
            <div id="sl-mr-settings">
                <div class="sl-mr-settings-head">
                    <div>
                        <div class="sl-mr-settings-title">🎯 SakaLuX Mission Rewards</div>
                        <div class="sl-mr-settings-sub">v${VERSION} • ${escapeHtml(state.apiMode || 'API not loaded')}</div>
                    </div>
                    <button id="sl-mr-settings-close">×</button>
                </div>
                ...
                <div class="sl-mr-api-box">
                    <div>API: <b>${getApiKey() ? '✅ Available' : '⚠️ Missing'}</b></div>
                    ${!getApiKey() ? '<input id="sl-mr-api-key" type="password" placeholder="Minimal Torn API key...">' : ''}
                </div>
    ...
    function injectCss() {
        ...
        style.textContent = `
            ...
            .sl-mr-card-info{margin-top:5px;padding:5px 6px;border-radius:6px;background:rgba(17,24,39,.9);color:#fff;font-size:9px;line-height:1.4;pointer-events:none}
            .sl-mr-line{font-weight:800}.sl-mr-line.good{color:#4ade80}.sl-mr-line.special,.sl-mr-line.special-text{color:#fbbf24}.sl-mr-line.muted{color:#9ca3af;font-weight:600}
            .sl-mr-detail{margin-top:10px;padding:10px;border-radius:9px;background:#111827;border:1px solid #303640;color:#e5e7eb;font-size:11px;line-height:1.6}
            .sl-mr-detail-title{color:#fbbf24;font-size:12px;font-weight:900;margin-bottom:6px}.sl-mr-highlight{margin-top:5px;color:#4ade80;font-size:12px}.sl-mr-note{margin-top:7px;color:#9ca3af;font-size:9px}
            #sl-mr-settings-overlay{position:fixed;inset:0;z-index:2147483647;background:rgba(0,0,0,.75);display:flex;align-items:flex-end;justify-content:center;font-family:Arial,sans-serif}
            #sl-mr-settings{width:min(560px,100%);max-height:90vh;overflow:auto;box-sizing:border-box;padding:14px;background:#101318;color:#fff;border-radius:18px 18px 0 0}
            .sl-mr-settings-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}.sl-mr-settings-title{font-size:17px;font-weight:900}.sl-mr-settings-sub{margin-top:3px;color:#9ca3af;font-size:9px}
            #sl-mr-settings-close{width:36px;height:36px;border:0;border-radius:9px;background:#252a32;color:#fff;font-size:20px}.sl-mr-setting{display:block;margin-bottom:7px;padding:10px;border-radius:9px;background:#181d24;border:1px solid #292f38;font-size:11px}
            .sl-mr-api-box{margin-top:10px;padding:10px;background:#181d24;border:1px solid #292f38;border-radius:9px;font-size:11px}#sl-mr-api-key{width:100%;box-sizing:border-box;margin-top:8px;padding:9px;border:1px solid #303640;border-radius:8px;background:#101318;color:#fff}
            .sl-mr-settings-btn{width:100%;margin-top:7px;min-height:40px;border:0;border-radius:9px;background:#2563eb;color:#fff;font-weight:900}.sl-mr-settings-btn.gray{background:#374151}
    ...
    function createButton() {
        if (document.getElementById('sl-mri-button')) return;
    ...
    window.SakaLuXMissionRewards = {
        ...
        health() {
            return {
                ...
                rewardCards: getRewardCards().length,
                lastScan: state.lastScan,
                learnedMods: Object.keys(getModRanges()).length
            };
        },
        goToMissions() {
            location.href = 'https://www.torn.com/page.php?sid=missions';
            return true;
        }
    };
