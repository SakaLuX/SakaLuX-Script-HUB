...
// @version      1.7.0
// @description  Central manager, installer, launcher and health monitor for SakaLuX Torn add-ons, with updates, quick actions, favorites, search, backup and mobile/PDA support.
// @author       SakaLuX
...
(function () {
    ...
    // Keep the V16 keys so existing Hub settings/favorites/usage survive this update.
    const STORAGE = {
    ...
    function getUpdateCount() {
        return SCRIPTS.filter(script => {
            const data = updateCache[script.id];
            return Boolean(getInstalledVersion(script) && data?.available);
        }).length;
    }
    ...
    function getMissingCount() {
        return SCRIPTS.filter(script => !script.api()).length;
    }

    function openInstall(script) {
        if (!script?.greasyFork?.downloadUrl) return;
        window.location.href = script.greasyFork.downloadUrl;
    }
    ...
    function updateBadge() {
        ...
        const total = healthErrors + getUpdateCount() + getMissingCount();
    ...
    function openHub() {
            ...
            <div class="slh-header">
                <div class="slh-headrow">
                    <div>
                        ...
                        <div class="slh-sub">v${VERSION} • Main script + optional SakaLuX add-ons</div>
                    </div>
                ...
                <div class="slh-tools">
                    ...
                    <button class="slh-tool" id="slh-update-check" title="Check Greasy Fork updates">⬆️</button>
                    <button class="slh-tool" id="slh-health" title="Health check">🩺</button>
        ...
        document.getElementById('slh-health').onclick = () => {
            updateBadge();
            renderMainStats();
            renderList();
        };
    ...
    function renderMainStats() {
        ...
        const installed = rows.filter(row => row.health.state !== 'missing').length;
        const healthy = rows.filter(row => row.health.state === 'ok').length;
        ...
        stats.innerHTML = `
            ...
            <div class="slh-stat"><strong>${updates}</strong><span>UPDATES</span></div>
            <div class="slh-stat"><strong>${errors + updateErrors}</strong><span>ISSUES</span></div>
        `;
    }
    ...
    function renderCategories() {
        ...
        box.innerHTML = categories.map(value => `
            <button class="slh-cat ${category === value ? 'active' : ''}" data-category="${escapeHtml(value)}">${escapeHtml(value)}</button>
        `).join('');
    ...
    function renderCard(row) {
        ...
        const installedVersion = getInstalledVersion(script);
        ...
        } else if (script.id === 'bazaar' && health.data) {
            const active = health.data.onEvents || health.data.onMessages;
            extra = active
        ...
        const actions = missing
            ...
            : `${update.state === 'available' ? `<button class="slh-action update" data-update="${escapeHtml(script.id)}">⬆ UPDATE</button>` : ''}${script.quickActions.map(action => `<button class="slh-action ${action.id === 'open' ? '' : 'secondary'}" data-script="${escapeHtml(script.id)}" data-action="${escapeHtml(action.id)}">${action.icon} ${action.label}</button>`).join('')}`;
            ...
            <div class="slh-card ${row.favorite ? 'favorite' : ''} ${update.state === 'available' ? 'update' : ''} ${missing ? 'missing' : ''}">
                ...
                <div>
                    <div class="slh-name">
                        ...
                        <button class="slh-star" data-fav="${escapeHtml(script.id)}">${row.favorite ? '★' : '☆'}</button>
                    </div>
                    <div class="slh-meta">
                        Installed: <b>${installedVersion ? 'v' + escapeHtml(installedVersion) : 'NOT INSTALLED'}</b>
                        • Latest: <b>${latest === '?' ? '?' : 'v' + escapeHtml(latest)}</b><br>
                        <span class="slh-update-status ${update.state}">${escapeHtml(update.text)}</span>
                        ${update.data?.checkedAt ? ' • Checked ' + escapeHtml(formatAgo(update.data.checkedAt)) : ''}<br>
    ...
    async function runAction(id, action) {
        ...
        try {
            ...
            if (typeof api[action] === 'function') {
                ...
                if (action === 'open') {
                    if (result === false && api.goToEvents) {
                        api.goToEvents();
                    } else {
                        closeHub();
                    }
            ...
            if (action === 'open' && script.fallbackOpen()) {
                recordUsage(id);
                closeHub();
                return;
            }
        ...
        createOverlay(`
            ...
            <div class="slh-quick">
                ${rows.map(row => {
                    ...
                    return `
                        <button class="slh-big-btn ${update.state === 'available' ? 'update' : ''}" data-quick-open="${row.script.id}">
                            ${row.script.icon} ${escapeHtml(row.script.name)} • ${escapeHtml(row.health.text)}
                        </button>
                        ${update.state === 'available' ? `<button class="slh-big-btn update" data-quick-update="${row.script.id}">⬆ UPDATE TO v${escapeHtml(update.data.latest)}</button>` : ''}
                    `;
                }).join('')}
    ...
    function openSettings() {
        createOverlay(`
            ...
            <div class="slh-settings">
                ...
                <div class="slh-setting"><label><input id="slhs-long" type="checkbox" ${settings.longPressQuickMenu ? 'checked' : ''}> Long press ☠️ opens Quick Menu</label></div>
                <div class="slh-setting"><label><input id="slhs-auto-updates" type="checkbox" ${settings.autoCheckUpdates ? 'checked' : ''}> Automatically check Greasy Fork updates</label><div style="margin-top:5px;color:#8b949e;font-size:9px;">Automatic checks use a 24-hour cache.</div></div>
                <div class="slh-setting">Button position<select id="slhs-position"><option value="top-right">Top right</option><option value="middle-right">Middle right</option><option value="bottom-right">Bottom right</option><option value="top-left">Top left</option></select></div>
        ...
        document.getElementById('slhs-save').onclick = () => {
            ...
            settings.autoCheckUpdates = document.getElementById('slhs-auto-updates').checked;
    ...
    function restoreSettings() {
        ...
        try {
            ...
            if (data.app !== 'SakaLuX Script Hub') throw new Error('Invalid backup');
    ...
    function resetHub() {
        ...
        localStorage.removeItem(STORAGE.updates);
