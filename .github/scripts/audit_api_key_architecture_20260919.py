...
for p in sorted(files):
    ...
    uses_api='api.torn.com' in s or '###PDA-APIKEY###' in s or 'TORN_API_KEY' in s or 'API_KEY' in s
    ...
    rows.append({
        ...
        'hub_global':'SakaLuXScriptHub' in s,
        'has_create_button':bool(re.search(r'CREATE[^\n]{0,80}API|createRequired(?:Api|Torn)Key|API_CREATE_URL|REQUIRED_API_KEY_URL',s,re.I)),
...
# Compute union of module create-key permissions, intentionally excluding Account Auditor and Hub.
union={}
for r in rows:
    if r['file'] in {'SakaLuX-Script-Hub.user.js','SakaLuX-Account-Auditor.user.js'}: continue
...
lines += ['', '## Hub shared-key coverage','',f"- Union excluding Account Auditor: `{json.dumps({k:sorted(v) for k,v in union.items()},ensure_ascii=False)}`",f"- Hub shared URL: `{json.dumps(shared_sets,ensure_ascii=False,default=list)}`",f"- Missing from Hub shared key: `{json.dumps(missing,ensure_ascii=False)}`",f"- Extra in Hub shared key versus discovered module create URLs: `{json.dumps(extra,ensure_ascii=False)}`",'', '## Account Auditor isolation','',f"- Reads Hub key markers: **{'YES (problem)' if aud['hub_storage'] or aud['hub_global'] else 'no'}**",f"- Own create URL discovered: **{'yes' if aud['create_urls'] else 'no'}**",f"- Own create/API control detected: **{'yes' if aud['has_create_button'] else 'no'}**",'', '## Findings','']
problems=[]
...
if aud['uses_api'] and not aud['has_create_button']: problems.append('Account Auditor uses API data but no create/API control was detected.')
for r in rows:
    ...
    if r['uses_api'] and r['file'] not in {'SakaLuX-Script-Hub.user.js','SakaLuX-Account-Auditor.user.js'} and not r['create_urls'] and ('api.torn.com' in (root/r['file']).read_text(encoding='utf-8',errors='ignore')):
        problems.append(f"{r['file']} appears to use Torn API but no module-specific create-key URL was discovered.")
if problems:
    lines.extend(f'- ❌ {x}' for x in problems)
else:
    lines.append('- ✅ Architecture matches the requested policy.')
