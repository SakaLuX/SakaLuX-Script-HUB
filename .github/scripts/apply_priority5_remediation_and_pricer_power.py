...
# ---------- Bazaar Smart Pricer: global power bridge ----------
...
bridge_marker = '/* SakaLuX Bazaar Smart Pricer Global Power Bridge — BEGIN */'
...
(() => {
  ...
  try {
    ...
    if (!bridge) {
      ...
      bridge.type = 'button';
      bridge.id = 'sakalux-module-bridge-bazaar-smart-pricer';
      bridge.hidden = true;
    ...
    bridge.onclick = () => {
      const action = bridge.dataset.action;
      if (action === 'on') writeEnabled(true);
      else if (action === 'off') writeEnabled(false);
      else if (action === 'toggle') writeEnabled(!readEnabled());
      sync();
    };
    sync();
...
'''
    if canonical_end not in s:
        raise SystemExit('Pricer canonical marker missing')
    s = s.replace(canonical_end, canonical_end + bridge, 1)

# Make page runtime use the same globally controllable state.
old_is = """    function isModuleEnabled() {\n        return GM_getValue(MODULE_ENABLED_KEY, true) !== false;\n    }"""
new_is = """    function isModuleEnabled() {\n        try {\n            const local = localStorage.getItem('SakaLuX_BAZAAR_SMART_PRICER_ENABLED');\n            if (local === '1') return true;\n            if (local === '0') return false;\n        } catch {}\n        return GM_getValue(MODULE_ENABLED_KEY, true) !== false;\n    }"""
if old_is in s:
    s = s.replace(old_is, new_is, 1)
elif 'SakaLuX_BAZAAR_SMART_PRICER_ENABLED' not in s:
    raise SystemExit('Pricer isModuleEnabled anchor missing')
...
# ---------- Hub: direct remediation actions ----------
...
if summary_anchor not in h:
    raise SystemExit('Hub health summary anchor missing')
if 'function getRemediationAction(script)' not in h:
    ...
    async function runRemediationAction(id) {
        ...
        if (remediation.kind === 'update' || remediation.kind === 'install') {
            const url = getInstallUrl(script);
            if (url) { location.href = url; return true; }
            throw new Error('Installer URL is unavailable.');
        }
        if (remediation.kind === 'enable') {
            await setModulePower(script.id, true);
            return true;
        }
        if (remediation.kind === 'recheck') {
            await checkScriptUpdate(script, true);
            renderList(); renderMainStats();
            return true;
        }
        if (remediation.kind === 'api') {
            const api = script.api();
            const apiAction = (script.quickActions || []).find(a => ['create-key','api-key','test-key'].includes(a.id))
                || (script.quickActions || []).find(a => /api|key/i.test(a.label || ''));
            if (api && apiAction && typeof api[apiAction.method] === 'function') {
                await api[apiAction.method]();
                return true;
            }
        ...
        if (remediation.kind === 'page') {
            const action = getPrimaryAction(script);
            const configured = (script.quickActions || []).find(a => a.id === action.id) || action;
...
# Bazaar Pricer power is always ready once the installed version marker exists,
# even when the full page runtime is not mounted.
...
if old_power_ready in h:
    h = h.replace(old_power_ready, new_power_ready, 1)

# Add remediation chip/button next to health chips.
...
if chip_anchor not in h:
    raise SystemExit('Hub card chip anchor missing')
...
if bind_anchor not in h:
    raise SystemExit('Hub bindCards anchor missing')
if "[data-remediate]" not in h:
    bind_extra = """        document.querySelectorAll('[data-remediate]').forEach(button => {\n            button.onclick = async () => {\n                button.disabled = true;\n                try { await runRemediationAction(button.dataset.remediate); }\n                catch (error) { console.error('[SakaLuX Hub remediation]', error); alert('Repair action failed: ' + String(error?.message || error)); renderList(); }\n            };\n        });\n"""
    h = h.replace(bind_anchor, bind_anchor + bind_extra, 1)

# Make Bazaar Smart Pricer power controllable even if the full API/bridge is absent.
...
new_else = """        } else {\n            const bridge = document.getElementById('sakalux-module-bridge-' + script.id);\n            if (bridge) {\n                bridge.dataset.action = enabled ? 'on' : 'off';\n                bridge.click();\n            } else if (script.id === 'bazaar-smart-pricer' && getInstalledVersion(script)) {\n                try { localStorage.setItem('SakaLuX_BAZAAR_SMART_PRICER_ENABLED', enabled ? '1' : '0'); } catch {}\n                try { window.dispatchEvent(new CustomEvent('SakaLuX:BazaarSmartPricerPowerRequested', { detail: { enabled: Boolean(enabled) } })); } catch {}\n            } else {\n                throw new Error(script.name + ' control interface is unavailable on this page.');\n            }\n        }\n"""
if old_else not in h:
    raise SystemExit('Hub setModulePower fallback anchor missing')
h = h.replace(old_else, new_else, 1)

# Listen in the Pricer global bridge for Hub power requests.
# This is done in the Pricer source text after its bridge creation.
p = Path('SakaLuX-Bazaar-Smart-Pricer.user.js')
s = p.read_text(encoding='utf-8')
listener_anchor = """    window.addEventListener('SakaLuX:BazaarSmartPricerStateChanged', sync, { passive: true });\n  } catch {}\n})();\n/* SakaLuX Bazaar Smart Pricer Global Power Bridge — END */"""
listener_repl = """    window.addEventListener('SakaLuX:BazaarSmartPricerStateChanged', sync, { passive: true });\n    window.addEventListener('SakaLuX:BazaarSmartPricerPowerRequested', event => { writeEnabled(event?.detail?.enabled); sync(); }, { passive: true });\n  } catch {}\n})();\n/* SakaLuX Bazaar Smart Pricer Global Power Bridge — END */"""
if listener_anchor in s:
    s = s.replace(listener_anchor, listener_repl, 1)
p.write_text(s, encoding='utf-8')
...
for row in reg.get('scripts', []):
    ...
    if row.get('id') == 'bazaar-smart-pricer':
        row['version'] = '1.1.12'
        row['detailsRevision'] = int(row.get('detailsRevision', 0)) + 1
        row['release'] = {'version':'1.1.12','date':'2026-09-26','notes':['Adds a global persistent power bridge so Hub ON/OFF works from every Torn page.','Synchronizes Hub power state with the Pricer userscript storage.','Keeps Bazaar pricing runtime page-scoped while power control remains global.']}
reg_path.write_text(json.dumps(reg, indent=2, ensure_ascii=False) + '\n', encoding='utf-8')

# Docs.
doc = Path('greasyfork/Script-Hub.md')
d = doc.read_text(encoding='utf-8')
d = d.replace('**v1.9.87**','**v1.9.88**',1)
d = re.sub(r'- Canonical version: \*\*v1\.9\.\d+\*\*','- Canonical version: **v1.9.88**',d,1)
...
for path in ['greasyfork/Bazaar-Smart-Pricer.md','greasyfork/Bazaar-Smart-Pricer.user.md']:
    q=Path(path)
    if not q.exists(): continue
    x=q.read_text(encoding='utf-8')
    x=x.replace('1.1.11','1.1.12')
    if 'v1.1.12' not in x:
...
Path('SakaLuX-Script-Hub.user.js').write_text(h,encoding='utf-8')
