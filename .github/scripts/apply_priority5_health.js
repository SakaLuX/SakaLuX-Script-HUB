...
const add=`        const diagnostics = collectHubDiagnostics();
        ...
        } else results.push({ level: 'warn', label: 'API Broker', detail: 'Shared Core diagnostics unavailable' });
`;
...
if(!hub.includes('diagnostics: collectHubDiagnostics')) hub=hub.replace(apiAnchor,"        refresh: async () => { await refreshRegistryAndCheck(); return true; },\n        diagnostics: collectHubDiagnostics,\n        health: () => ({ ready: true, version: VERSION");
fs.writeFileSync(hubPath,hub);
const docPath='greasyfork/Script-Hub.md';
let doc=fs.readFileSync(docPath,'utf8');
doc=doc.replace('**v1.9.85**','**v1.9.86**');
doc=doc.replace(/- Canonical version: \*\*v1\.9\.\d+\*\*/,'- Canonical version: **v1.9.86**');
doc=doc.replace(/\*\*v1\.9\.85 — Persistent native Fly-out launcher \+ release synchronization\*\*[\s\S]*?(?=\n## Release history)/,'**v1.9.86 — Hub Health / Diagnostics**\n- Adds consolidated module health, disabled-module and runtime-error diagnostics.\n- Adds Shared Core API Broker counters and transport status without exposing API keys.\n- Exposes `SakaLuXScriptHub.diagnostics()` for safe local diagnostics.\n');
if(!doc.includes('### v1.9.86 — Hub Health / Diagnostics')) {
 ...
 const idx=doc.indexOf('\n### v'); doc=idx>=0?doc.slice(0,idx)+section+doc.slice(idx):doc+section;
}
