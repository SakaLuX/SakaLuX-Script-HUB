...
s=repl(s,"    function parseMoney(text) { const s=String(text||'').replace(/[^0-9]/g,''); return s?Number(s):NaN; }",
...
s=repl(s,"    function extractFirstPrice(node) { const txt=(node?.innerText||node?.textContent||'').replace(/\\s+/g,' '); const m=txt.match(/\\$\\s*([\\d,.]+)/); return m?parseMoney(m[1]):NaN; }",
"    function extractFirstPrice(node) { const txt=(node?.innerText||node?.textContent||'').replace(/\\s+/g,' '); const m=txt.match(/\\$\\s*([\\d,.]+)\\s*([KMB])?/i); return m?parseMoney(m[1]+(m[2]||'')):NaN; }",'compact row price')
...
s=repl(s,"            if(projected!=null&&e.qty&&expectedRestocks>0) projected+=e.qty*expectedRestocks;",
"            if(projected!=null&&e.qty&&expectedRestocks>0) projected=Math.min(MAX_REASONABLE_TRAVEL_STOCK,projected+e.qty*expectedRestocks);",'projected stock clamp')

s=repl(s,"        return {current:projected==null?null:stock,projected,expectedRestocks,confidence,eta:e};",
