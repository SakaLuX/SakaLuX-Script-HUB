...
const V='1.2.20',ID='chat-intelligence',API='SakaLuXChatIntelligence';
...
function decorate(r,e,first){const p=who(e),b=body(e,p),k=e.dataset.messageId||e.id||H((p.id||p.name)+'|'+b);if(!BOUND.has(e)){BOUND.add(e);e.onclick=x=>{if(!x.target.closest('a,button,input,textarea,[contenteditable="true"]'))showMenu(r,e,p)}}if(MESSAGE_KEYS.get(e)===k)return;MESSAGE_KEYS.set(e,k);if(first||!p.name||!allow(r)||M.has(pk(p))){rememberMessage(k);return}toast(r,(F.has(pk(p))?'★ ':'')+dn(p),b,k)}
...
(()=>{
 const mount=()=>{
  ...
  Object.assign(e.dataset,{id:'chat-intelligence',name:'Chat',icon:'💬',selector:'',fallback:'https://www.torn.com/index.php',version:'1.2.22'});
 };
