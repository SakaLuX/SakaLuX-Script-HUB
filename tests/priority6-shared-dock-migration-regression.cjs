...
for(const file of targets){
  ...
  assert.ok(!src.includes('SakaLuX Standalone Dock Bootstrap — BEGIN'),`${file}: legacy bootstrap removed`);
  assert.ok(!src.includes('data-slx-standalone-registration'),`${file}: legacy DOM registration removed`);
